function removeNoise(e, type) {
    let inputNoise;

    if (type == "input") {
        inputNoise = e.offsetParent.offsetParent.lastElementChild;
        e.offsetParent.offsetParent.classList.remove("is-focused");
    } else {
...
function createSvg(config) {
    let svgGroup = document.createElementNS("http://www.w3.org/2000/svg", "svg");
...
function generateNoise(e, type) {
    ...
    if (type == "input") {
        ...
        e.offsetParent.offsetParent.classList.add("is-focused");
        inputNoise = e.offsetParent.offsetParent.lastElementChild;
